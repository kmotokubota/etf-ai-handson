...
GlobalX ETFのポートフォリオデータ・ファンドドキュメント・マーケットニュースを統合し、
...
3. `GLOBALX_HANDSON_DB` / `DEMO_SCHEMA` と各テーブル・ウェアハウスが作成される
...
3. 以下を入力する
   ...
   | 項目 | 値 |
   |---|---|
   | URL | `https://github.com/kmotokubota/globalx-etf-ai-handson` |
   | 認証 | **パブリックリポジトリ**として作成 |
...
GlobalXのインカム系ETFの一覧と、それぞれの特徴・配当利回りを比較してください。
```
...
| Cortex Agent | `GLOBALX_PORTFOLIO_COPILOT` | 統合AIアシスタント（手動作成） |
